...
lambda_client = boto3.Session(profile_name=AWS_PROFILE, region_name=AWS_REGION).client(
    "lambda"
)
...
def format_lambda_response(function_name: str, payload: bytes) -> str:
    ...
    try:
        ...
        return (
            f"Function {function_name} returned: {json.dumps(payload_json, indent=2)}"
        )
...
def filter_functions_by_tag(functions, tag_key, tag_value):
    ...
    logger.info(
        f"{len(tagged_functions)} Lambda functions found with tag {tag_key}={tag_value}."
    )
...
def register_lambda_functions():
    ...
    try:
        ...
        if FUNCTION_PREFIX or FUNCTION_LIST:
            ...
            logger.info(
                f"{len(valid_functions)} Lambda functions found after name filtering."
            )
        else:
